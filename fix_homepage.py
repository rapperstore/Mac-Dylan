...
new_css = """
/* ── TIP JAR FLOAT ── */
.tip-float{position:fixed;top:68px;right:16px;z-index:500;display:flex;flex-direction:column;align-items:center;gap:3px;text-decoration:none;user-select:none}
.tip-jar-bg{width:44px;height:44px;background:rgba(5,4,3,.92);border:1px solid var(--ember);display:flex;align-items:center;justify-content:center;position:relative;overflow:visible;transition:border-color .2s,box-shadow .2s;animation:tfloat 3s ease-in-out infinite}
.tip-float:hover .tip-jar-bg{border-color:var(--fire);box-shadow:0 0 18px rgba(200,75,10,.55)}
.tip-jar-emoji{font-size:24px;line-height:1;display:block}
.tip-coin{position:absolute;width:7px;height:7px;border-radius:50%;background:var(--flame);opacity:0;animation:tcoin 2.2s ease-in-out infinite}
.tip-coin:nth-child(2){animation-delay:.35s}.tip-coin:nth-child(3){animation-delay:.7s}
.tip-float-lbl{font-family:'Space Mono',monospace;font-size:6px;letter-spacing:.2em;color:var(--ember);text-transform:uppercase;background:rgba(5,4,3,.9);padding:2px 5px;border:1px solid rgba(200,75,10,.3)}
...
/* ── SOCIAL BAR ── */
.social-bar{padding:36px 48px 52px;border-top:1px solid var(--border);display:flex;flex-direction:column;align-items:center;gap:16px}
...
.social-bar-row{display:flex;gap:10px;flex-wrap:wrap;justify-content:center}
.sb-btn{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:1px solid rgba(200,75,10,.45);background:rgba(200,75,10,.07);transition:all .22s;color:var(--ember);text-decoration:none;flex-shrink:0}
.sb-btn svg{width:14px;height:14px;fill:currentColor;transition:fill .2s}
.sb-btn:hover{background:var(--ember);border-color:var(--fire);box-shadow:0 0 14px rgba(200,75,10,.45)}
.sb-btn:hover svg{fill:var(--white)}
@media(max-width:900px){.tip-float{top:62px;right:10px}.social-bar{padding:28px 24px 40px}}
"""
